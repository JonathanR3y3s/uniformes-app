...
function execRenderDashboardContent(){
  ...
  if(dot.available){
    ...
    html+=execKpi({label:'Cobertura dotación',value:'',available:false,sub:'No disponible',note:'Requiere dotaciones configuradas',status:'warning',icon:'fa-clipboard-check'});
    html+=execKpi({label:'Avance entrega dotación',value:'',available:false,sub:'No disponible',note:'Requiere registros de entrega de dotación',status:'warning',icon:'fa-box-open'});
  }
  html+='</div>';
  html+='<div class="exec-two-col"><div class="exec-panel"><div class="exec-panel-head"><div><h3>Top 5 productos con mayor consumo</h3><p>Click en un producto para ver movimientos</p></div></div>'+execRenderTopList(data.topConsumo,'qty')+'</div><div class="exec-panel"><div class="exec-panel-head"><div><h3>Top 5 productos con mayor costo</h3><p>Mayor valor financiero en inventario actual</p></div></div>'+execRenderTopList(data.topCosto,'cost')+'</div></div>';
  html+=execRenderCharts(data);
